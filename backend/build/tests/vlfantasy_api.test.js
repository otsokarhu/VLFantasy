...
beforeEach(async () => {
    await (0, testhelper_1.default)();
});
describe('login tests', () => {
    beforeEach(async () => {
        const passwordHash = await bcrypt_1.default.hash('testpassword', 10);
...
describe('runner tests', () => {
    ...
    test('there are six runners', async () => {
        ...
        expect(response.body).toHaveLength(testhelper_1.initialRunners.length);
    });
    test('every runner has an id', async () => {
        const response = await api.get('/api/runners');
        response.body.forEach((runner) => {
    ...
    test('runner can be added', async () => {
        ...
        await api
            ...
            .expect('Content-Type', /application\/json/);
        const response = await api.get('/api/runners');
        expect(response.body).toHaveLength(testhelper_1.initialRunners.length + 1);
    });
    test('runner can be deleted', async () => {
        ...
        expect(responseAfterDelete.body).toHaveLength(testhelper_1.initialRunners.length - 1);
...
describe('fantasy team tests', () => {
    ...
    test('fantasy team can be added with auth', async () => {
        const passwordHash = await bcrypt_1.default.hash('sikret', 10);
        const user = new userModel_1.default({
            name: 'Test User',
            username: 'testuser',
            email: 'test',
        ...
        const teamNames = response.body.map((team) => team.name);
...
describe('user tests', () => {
    ...
    test('username can be changed', async () => {
        ...
        const usernames = usersAfterChange.body.map((user) => user.username);
    ...
    test('name can be changed', async () => {
        ...
        await api
            ...
            .expect(200);
        const usersAfterChange = await api.get('/api/VLusers');
        const names = usersAfterChange.body.map((user) => user.name);
    ...
    test('username can be changed', async () => {
        ...
        await api
            ...
            .expect(200);
        const usersAfterChange = await api.get('/api/VLusers');
        const emails = usersAfterChange.body.map((user) => user.email);
